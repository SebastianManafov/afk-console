...
Bot POV right-clicks are routed against Mineflayer's live cursor target: an entity is activated first, then a block, then the held item. Server-opened windows use a separate authenticated bridge. Mineflayer's window ID, type, title, inventory boundary and complete slot array are forwarded on open, slot updates and close; the browser renders supported chest, hopper, dispenser/dropper and furnace-family windows with the pinned 1.21.4 GUI textures and existing item sprites. GUI clicks are still lease-bound and admin-only, but pointer lock is released while the real server window is open so normal left, right and shift clicks can be sent back through `bot.clickWindow`.
