...
Pose diagnostics are disabled in normal operation. Set `RCC_POSE_DIAGNOSTICS=true` for server-side `[POSE]` traces, and add `?poseDiagnostics=1` to the viewer URL only when browser-side model tracing is needed.
